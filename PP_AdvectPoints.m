...
if nargin == 5 && strcmpi(varargin{1},'singlestep')
    ...
    for k = 1:n-1
        ...
        K4v = interp2(VEC{k+1}.X, VEC{k+1}.Y, VEC{k+1}.V, ...
            A.X(c,:) + dt*K3u, A.Y(c,:) + dt*K3v, 'cubic');
        %% The part applies the preiodic bounday advection of
        %  the BickleyJet flow
        if nargin == 5 && strcmpi(varargin{2},'BickleyJet')
            Ind = find(A.X(c,:)<1);
            for m = 1 : length(Ind)
                if  isnan(K1u(Ind(m)) + 2*K2u(Ind(m)) + 2*K3u(Ind(m)) + K4u(Ind(m))) %|| isnan(K1v(Ind(m)) + 2*K2v(Ind(m)) + 2*K3v(Ind(m)) + K4v(Ind(m)))
                    A.X(c,Ind(m)) = 20;
                    K1u(Ind(m)) = interp2(VEC{k}.X, VEC{k}.Y, VEC{k}.U, A.X(c,Ind(m)), A.Y(c,Ind(m)), ...
                        'cubic');
                    K1v(Ind(m)) = interp2(VEC{k}.X, VEC{k}.Y, VEC{k}.V, A.X(c,Ind(m)), A.Y(c,Ind(m)), ...
                        'cubic');
                    K2u(Ind(m)) = interp2(Xhalf, Yhalf, Uhalf, A.X(c,Ind(m)) + 0.5*dt*K1u(Ind(m)), ...
                        A.Y(c,Ind(m)) + 0.5*dt*K1v(Ind(m)), 'cubic');
                    K2v(Ind(m)) = interp2(Xhalf, Yhalf, Vhalf, A.X(c,Ind(m)) + 0.5*dt*K1u(Ind(m)), ...
                        A.Y(c,Ind(m)) + 0.5*dt*K1v(Ind(m)), 'cubic');
                    K3u(Ind(m)) = interp2(Xhalf, Yhalf, Uhalf, A.X(c,Ind(m)) + 0.5*dt*K2u(Ind(m)), ...
                        A.Y(c,Ind(m)) + 0.5*dt*K2v(Ind(m)), 'cubic');
                    K3v(Ind(m)) = interp2(Xhalf, Yhalf, Vhalf, A.X(c,Ind(m)) + 0.5*dt*K2u(Ind(m)), ...
                        A.Y(c,Ind(m)) + 0.5*dt*K2v(Ind(m)), 'cubic');
                    K4u(Ind(m)) = interp2(VEC{k+1}.X, VEC{k+1}.Y, VEC{k+1}.U, ...
                        A.X(c,Ind(m)) + dt*K3u(Ind(m)), A.Y(c,Ind(m)) + dt*K3v(Ind(m)), 'cubic');
                    K4v(Ind(m)) = interp2(VEC{k+1}.X, VEC{k+1}.Y, VEC{k+1}.V, ...
                        A.X(c,Ind(m)) + dt*K3u(Ind(m)), A.Y(c,Ind(m)) + dt*K3v(Ind(m)), 'cubic');
